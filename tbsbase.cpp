...
SOCKET TBSbase::udpOpen(QString ipaddr,
                        int ipport)
{
    SOCKET ufd;
...
int TBSbase::udpOpen(QString ipaddr, int ipport)
{
    ...
    char recvbuff[64] = { 0 };
    /*create Socket*/
    if ((ufd = socket(AF_INET, SOCK_DGRAM,
                      0)) == -1) {
    ...
    udpsockaddr.sin_addr.s_addr = inet_addr(
                                      ipaddr.toStdString().data());
    len = sizeof(udpsockaddr);
    setsockopt(ufd, SOL_SOCKET, SO_RCVTIMEO,
               (char *)&timeout, sizeof(timeout));
    setsockopt(ufd, SOL_SOCKET, SO_SNDTIMEO,
               (char *)&timeout, sizeof(timeout));
    return ufd;
}
...
#ifdef Q_OS_WIN //windows
SOCKET TBSbase::udpOpen_no(QString ipaddr,
                           int ipport)
{
    SOCKET ufd;
    WSAStartup(MAKEWORD(1, 1), &wsaData);
...
int TBSbase::udpOpen_no(QString ipaddr,
                        int ipport)
{
    int ufd = 0;
...
int TBSbase::readREG64ByUDP(int subAddr,
                            unsigned char num, unsigned char *rdbuffer)
{
    int n = 0;
    int i = 0;
    int j = 0;
    ...
    if ((recvbuff[1] & 0xff) != 0x88) { //check ACK;
        qDebug() <<
                 QString("%1->%2->%3:recvfrom ACK failed")
                 .arg(__FILE__).arg(__LINE__).arg(__FUNCTION__);
        return -1;
    }
...
int TBSbase::writeREG64ByUDP(int subAddr,
                             unsigned char num, unsigned char *wtbuffer)
{
    int n = 0;
    int i = 0;
    int j = 0;
    ...
    len = sizeof(udpsockaddr);
    // 27bit address
    sendbuff[0] = 0x00 + (u8)((subAddr & 0x07000000)
                              >> 24);
    sendbuff[1] = (u8)((subAddr & 0x00ff0000) >> 16);
    sendbuff[2] = (u8)((subAddr & 0x0000ff00) >> 8);
    sendbuff[3] = (u8)((subAddr & 0x000000ff));
...
int TBSbase::readREG32FromExternalMemoryOnce(
    int m_addr,
    ...
    unsigned char num)
{
    ...
    if (-1 == checkHostStatus(1)) {
        qDebug() <<
                 QString("%1->%2->%3:wait for host free timeout")
                 .arg(__FILE__).arg(__LINE__).arg(__FUNCTION__);
        return -1;
    }
    //...wt data & cs to low-->...................
    ...
    if (-1 == checkHostStatus(0)) {
        qDebug() <<
                 QString("%1->%2->%3:wait for host active timeout!")
                 .arg(__FILE__).arg(__LINE__).arg(__FUNCTION__);
    ...
    if (-1 == waitForHostWorkDone()) {
        qDebug() <<
                 QString("%1->%2->%3:wait host work done timeout!")
                 .arg(__FILE__).arg(__LINE__).arg(__FUNCTION__);
        return -1;
    }
    if (readREG(mode, 0x4000 + 1 * 4, 4,
                &buff[0]) < 0) {
        return -1;
    }
    if (readREG(mode, 0x4000 + 2 * 4, 4,
                &buff[4]) < 0) {
        return -1;
    }
    for (i = 0; i < num; i++)
...
int TBSbase::writeREG32ToExternalMemoryOnce(
    int m_addr,
    ...
    unsigned char num)
{
    ...
    if (writeREG(mode, 0x4000 + 2 * 4, 4,
                 &buff[4]) < 0) {
        return -1;
    }

    if (writeREG(mode, 0x4000 + 1 * 4, 4,
                 &buff[0]) < 0) {
        return -1;
    }
                                        ...
                                        unsigned char num)
{
    ...
    case REG32_TO_OR_FromExtMemOnce:
        return readREG32FromExternalMemoryOnce(m_addr,
                                               rdbuff, num);
        break;
    default:
        break;
                                       ...
                                       unsigned char num)
{
    switch (mode) {
    case REG32_TO_OR_FromExtMemOnce:
        return writeREG32ToExternalMemoryOnce(m_addr,
                                              wtbuff, num);
        break;
    default:
        break;
                                    ...
                                    int rd_size)
{
    ...
    const int x8byte = 8;
    if (rd_size <= 8) {
        return readFromExternalMemoryOnce(mode, ram_addr,
                                          &rdbff[0], rd_size);
    } else if (rd_size > 8) {
        k = rd_size / x8byte;
        g = rd_size % x8byte;
        for (i = 0; i < k; i++) {
            ret = readFromExternalMemoryOnce(mode,
                                             (ram_addr + i * 8), &rdbff[i * 8], x8byte);
            if (-1 == ret) {
                return ret;
            }
        }
                                   ...
                                   int wt_size)
{
    ...
    const int x8byte = 8;
    if (wt_size <= 8) {
        return writeToExternalMemoryOnce(mode, ram_addr,
                                         wtbff, wt_size);
    } else if (wt_size > 8) {
        k = wt_size / x8byte;
        g = wt_size % x8byte;
        for (i = 0; i < k; i++) {
            ret = writeToExternalMemoryOnce(mode,
                                            (ram_addr + i * 8), &wtbff[i * 8], x8byte);
            if (-1 == ret) {
                return ret;
            }
        }
                                   ...
                                   int size)
{
    if (READ == mode) {
        ...
        qDebug() <<
                 QString("%1->%2->%3:read write mode error!")
                 .arg(__FILE__).arg(__LINE__).arg(__FUNCTION__);
    }
    return 0;
}

int TBSbase::readREG32MonopolizeCPUStatus(
    int m_addr,
    unsigned char *rdbuff,
    unsigned char num)
{
...
int TBSbase::writeREG32MonopolizeCPUStatus(
    int m_addr, unsigned char *wtbuff,
    unsigned char num)
{
    int i = 0;
    int mode = REG64_BY_UDP_FUNC;
