...
int SetMACTool::initUI(void)
{
    ...
    connect(&m_Thread, SIGNAL(started()), tbshd,
            SLOT(start()));
    connect(tbshd, SIGNAL(sigDisplayMsgUI(
                              TBS_Msg_Type *)),
...
void SetMACTool::soltsDisplayMsgUI(
    TBS_Msg_Type *msg)
{
    if (NULL == msgbox) {
        return;
    ...
    if (1 == msg->type) {
        ...
        ui->com_LocalIP->addItem(nettag[netnum].ip,
                                 netnum);
        ...
        if ((1 == msg->isread)) {
            if (0 == com_IP_soltEN_flg) {
                connect(ui->com_LocalIP,
                        SIGNAL(currentIndexChanged(int)),
                        this, SLOT(slot_com_LocalIP_currentIndexChanged(
                                       int)));
                com_IP_soltEN_flg = 1;
            }
...
void SetMACTool::on_btn_Macapply_clicked()
{
    ...
    qDebug() << "idx:" << ipindex <<
             nettag[ipindex].ip << nettag[ipindex].port;
    if (-1 == ipindex) {
        return;
    }
...
void SetMACTool::slot_com_LocalIP_currentIndexChanged(
    int idx)
{
#if 1
    if (idx < 0) {
