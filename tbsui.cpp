...
tbsui::~tbsui()
{
    if ( tbshd ) {
        //tbshd->stop();
        connect ( &m_Thread, SIGNAL ( finished() ), tbshd,
                  SLOT ( deleteLater() ) );
...
void tbsui::init_sql ( void )
{
    ...
    QString create_sql =
        "create table macValue (id int,MAC varchar(20))";
...
{
    QString update_sql =
        "update macValue set MAC = :MAC where id = :id";
    sql_query->prepare ( update_sql );
    sql_query->bindValue ( ":MAC",
                           qstrMac.toStdString().data() );
...
void tbsui::refresh ( void )
{
    netnum = 0;
    ui->com_IP->clear();
    disconnect ( ui->com_IP,
                 SIGNAL ( currentIndexChanged ( int ) ), this,
                 SLOT ( slot_com_IP_currentIndexChanged (
                            int ) ) );
    com_IP_soltEN_flg = 0;
...
bool tbsui::eventFilter ( QObject *watched,
                          QEvent *event )
{
    if ( event->type() ==
...
void tbsui::initForm()
{
    ...
    ui->labTitle->setFont ( QFont ( "Microsoft Yahei",
                                    20 ) );
    ...
    qss.append (
        QString ( "QComboBox::down-arrow,QDateEdit[calendarPopup=\"true\"]::down-arrow,QTimeEdit[calendarPopup=\"true\"]::down-arrow,"
            ...
            addHeight ) );
    qss.append (
        QString ( "QRadioButton::indicator{width:%1px;height:%1px;}" ).arg (
            rbtnWidth ) );
    qss.append (
    ...
    qss.append (
        QString ( "QWidget#widget_top>QToolButton:pressed,QWidget#widget_top>QToolButton:hover,"
                  "QWidget#widget_top>QToolButton:checked,QWidget#widget_top>QLabel:hover{"
                  "border-width:0px 0px %1px 0px;}" ).arg (
            borderWidth ) );
    qss.append (
        QString ( "QWidget#widgetleft>QPushButton:checked,QWidget#widgetleft>QToolButton:checked,"
                  "QWidget#widgetleft>QPushButton:pressed,QWidget#widgetleft>QToolButton:pressed{"
                  "border-width:0px 0px 0px %1px;}" ).arg (
            borderWidth ) );
    this->setStyleSheet ( qss.join ( "" ) );
    //  connect(ui->tree_Dev, SIGNAL(itemClicked(QTreeWidgetItem*, int)), this, SLOT(showSelectedImage(QTreeWidgetItem*, int)));
    QList<QCheckBox *> ches =
        ui->gbox_Tuners->findChildren<QCheckBox *>();

    foreach ( QCheckBox *che, ches ) {
        che->setCheckable ( true );
        connect ( che, SIGNAL ( clicked() ), this,
                  SLOT ( tunersCheckboxClick() ) );
    }
    ...
    connect ( &m_Thread, SIGNAL ( started() ), tbshd,
              SLOT ( start() ) );
    connect ( tbshd,
              SIGNAL ( sigDisplayMsgUI ( TBS_Msg_Type * ) ),
              this, SLOT ( soltsDisplayMsgUI (
                               TBS_Msg_Type * ) ) );
    tbsrwparm.devno = devno;
    tbshd->setHardWareParm ( tbsrwparm );
    tbshd->setRunMode ( TBS_UDPMULTICAST_FUNC );
...
void tbsui::on_too_Apply_clicked()
{
    ...
    uiudpfd = tbshd->udpOpen ( nettag[ipindex].ip,
                               nettag[ipindex].port );
    ...
    if ( uiudpfd < 3 ) {
        ...
        if ( 0 == index ) {
            ...
            if ( 0 == ( ( nettag[ipindex].switchStatus >>
                          devno ) & 0x01 ) ) {
                //check ui
                int sym = ui->lin_Sym->text().toInt();//2000~7200
                int tsport = ui->lin_TSPort->text().toInt();
                float fre =
                    ui->lin_Fre->text().toFloat();//100.000~1000.000
                float lev =
                    ui->lin_Lev->text().toFloat();//0.0~-35.0
                ...
                if ( ( tbsrwparm.qam ==
                       ui->com_Modulation->currentIndex() )
                     && ( tbsrwparm.sym ==
                          ui->lin_Sym->text().toInt() ) ) {
                    tbsrwparm.isQam_sym_changed = 0;
                }
                ...
                tbsrwparm.protocol = ( u8 ) (
                                         ui->com_Protocol->currentIndex() );
                tbsrwparm.qam =
                    ui->com_Modulation->currentIndex();
...
void tbsui::slot_com_IP_currentIndexChanged (
    int idx )
{
#if 1
    ...
    qDebug() << "idx" << idx << nettag[idx].ip <<
             nettag[idx].port;
    uiudpfd = tbshd->udpOpen ( nettag[idx].ip,
                               nettag[idx].port );
...
void tbsui::soltsDisplayMsgUI ( TBS_Msg_Type *
                                msg )
{
    if ( NULL == msgbox ) {
        return;
    ...
    if ( 1 == msg->type ) {
        if ( ( 1 == msg->isread )
            ...
            if ( 0 == com_IP_soltEN_flg ) {
                connect ( ui->com_IP,
                          ...
                          this, SLOT ( slot_com_IP_currentIndexChanged (
                                           int ) ) );
                com_IP_soltEN_flg = 1;
            }
            ...
            if ( 0 == twindex ) {
                ...
                if ( 0 == dhcpEN ) {
                    ...
                    nettag[comindex].port =
                        ui->lin_Lport->text().toInt();
                    ui->com_IP->setItemText ( comindex,
                                              nettag[comindex].ip );
                }
        ...
        msgbox->hideBtn ( msg->btnL, msg->btnR,
                          msg->btnRtext );
        ...
        qDebug() <<
                 QString ( "%1->%2->%3:msg->switchStatus=%4" )
                 .arg ( __FILE__ )
                 .arg ( __LINE__ )
                 .arg ( __FUNCTION__ )
        ...
        if ( 0 == netnum ) {
            ui->che_t0->setDisabled ( ( msg->switchStatus >>
                                        0 ) & 0x01 );
            ui->che_t1->setDisabled ( ( msg->switchStatus >>
                                        1 ) & 0x01 );
            ui->che_t2->setDisabled ( ( msg->switchStatus >>
                                        2 ) & 0x01 );
            ui->che_t3->setDisabled ( ( msg->switchStatus >>
                                        3 ) & 0x01 );
        }

        qDebug() << "netnum:" << netnum << msg->devip <<
                 msg->devport;
