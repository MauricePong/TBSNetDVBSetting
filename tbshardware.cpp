...
void TBShardware::start()
{
    ...
    tbsmsg->displaytext =
        QString ( "Device running now,please wait.." );
    ...
    if ( 0 == ret ) { //read success
        ...
        tbsmsg->tilie = QString ( "information" );
        tbsmsg->displaytext =
            QString ( "Configuring now,please wait.." );
        tbsmsg->devip = QString ( "" );
        tbsmsg->devport = 0;
        tbsmsg->switchStatus = 0x0f;
        emit sigDisplayMsgUI ( tbsmsg );
...
int TBShardware::writeBuffer ( int wmode )
{
    ...
    case WRITE_MODULATOR_PARM_FUNC:
        ...
        if ( -1 == ret ) {
            ...
            writeREG ( REG64_BY_UDP_FUNC, 0x4034, 1,
                       writeMcudata );
        }
...
{
    u8 tmp[8] = { 0 };
    int ret = readREG ( REG64_BY_UDP_FUNC, 0x4014, 8,
                        tmp );
    //for (int i = 0; i < 8; i++) {
...
{
    u8 tmp[27] = { 0 };
    int ret = controlExternalMemory ( READ, 0x0000,
                                      tmp, 27 );
    //for (int i = 0; i < 27; i++) {
    //  qDebug() << tmp[i];
    //}
...
int TBShardware::readModulatorParm ( void )
{
    ...
    int procotoladdr[4] = { 0x0050, 0x0058, 0x0060, 0x068 };
    u8 tsportdata[2] = { 0, 0 };
    ...
    writeMcudata[0] = 1;
    ret = writeREG ( REG64_BY_UDP_FUNC, 0x4034, 1,
                     writeMcudata );
    ...
    QMSLEEP ( delaytime );
    ret = writeREG ( REG64_BY_UDP_FUNC, 0x0030, 1,
                     ( u8 * ) &rwparm.devno );
    ...
    ret = writeREG ( REG64_BY_UDP_FUNC, 0x0040, 5,
                     &tmp[8] );
    ...
    ret = writeREG ( REG64_BY_UDP_FUNC, 0x0038, 8,
                     &tmp[0] );
    ...
    memset ( tmp, 0, 32 );
    ret = controlExternalMemory ( READ,
                                  0x20 + 0x20 * rwparm.devno, tmp, 32 );
    ...
    QString qfreq = QString ( "%1%2%3%4%5%6%7" )
                    .arg ( ( char ) ( tmp[0x00] ) ).arg ( ( char ) (
                                tmp[0x01] ) )
                    .arg ( ( char ) ( tmp[0x02] ) ).arg ( ( char ) (
                                tmp[0x03] ) )
                    .arg ( ( char ) ( tmp[0x04] ) ).arg ( ( char ) (
                                tmp[0x05] ) )
                    .arg ( ( char ) ( tmp[0x06] ) );
    ...
    rwparm.pla = QString ( "%1" ).arg ( * ( int * ) (
                                            &playeratedata[0] ) );
    QString qlevl = QString ( "%1%2%3%4%5" )
                    .arg ( ( char ) ( tmp[0x15] ) ).arg ( ( char ) (
                                tmp[0x16] ) )
                    .arg ( ( char ) ( tmp[0x17] ) ).arg ( ( char ) (
                                tmp[0x18] ) )
                    .arg ( ( char ) ( tmp[0x19] ) );
    rwparm.lev = QString::number ( qlevl.toFloat(),
                                   'f', 1 );
    return ret;
}
...
int TBShardware::readNetMac ( void )
{
    u8 tmp[8] = { 0 };
    u8 mac[8] = { 0 };
    int ret = controlExternalMemory ( READ, 0x0008,
                                      tmp, 6 );
...
{
    u8 tmp[30] = { 0 };
    tmp[0x00] = rwparm.ip.section ( '.', 0,
                                    0 ).trimmed().toInt();
    tmp[0x01] = rwparm.ip.section ( '.', 1,
                                    1 ).trimmed().toInt();
    tmp[0x02] = rwparm.ip.section ( '.', 2,
                                    2 ).trimmed().toInt();
    tmp[0x03] = rwparm.ip.section ( '.', 3,
                                    3 ).trimmed().toInt();
    tmp[0x04] = ( ( rwparm.port >> 8 ) & 0xff );
    tmp[0x05] = ( ( rwparm.port ) & 0xff );
    int ret = controlExternalMemory ( WRITE, 0x0000,
                                      tmp, 6 );
    ...
    tmp[0x10] = rwparm.Netmask.section ( '.', 0,
                                         0 ).trimmed().toInt();
    tmp[0x11] = rwparm.Netmask.section ( '.', 1,
                                         1 ).trimmed().toInt();
    tmp[0x12] = rwparm.Netmask.section ( '.', 2,
                                         2 ).trimmed().toInt();
    tmp[0x13] = rwparm.Netmask.section ( '.', 3,
                                         3 ).trimmed().toInt();
    tmp[0x14] = 0;
    tmp[0x15] = 0;
    tmp[0x16] = rwparm.gateway.section ( '.', 0,
                                         0 ).trimmed().toInt();
    tmp[0x17] = rwparm.gateway.section ( '.', 1,
                                         1 ).trimmed().toInt();
    tmp[0x18] = rwparm.gateway.section ( '.', 2,
                                         2 ).trimmed().toInt();
    tmp[0x19] = rwparm.gateway.section ( '.', 3,
                                         3 ).trimmed().toInt();
    tmp[0x1a] = rwparm.dhcp;
    ret = controlExternalMemory ( WRITE, 0x0010,
                                  &tmp[0x10], 11 );
    ...
    //mcu rst
    memset ( tmp, 0xff, 8 );
    ret = writeREG ( REG64_BY_UDP_FUNC, 0x4014, 8,
                     tmp );
    QMSLEEP ( 10 );
    tmp[0] = 0;
    ret = writeREG ( REG64_BY_UDP_FUNC, 0x4014, 8,
...
int TBShardware::writeModulatorParm ( void )
{
    ...
    ret = writeREG ( REG64_BY_UDP_FUNC,
    ...
    ret = writeREG ( REG64_BY_UDP_FUNC,
                     procotoladdr[rwparm.devno], 1, &rwparm.protocol );
    ...
    if ( 1 == rwparm.isFreq_changed ) {
        ...
        ret = writeREG ( REG64_BY_UDP_FUNC, 0x0048, 8,
                         &tmp[16] );
        ...
        ret = writeREG ( REG64_BY_UDP_FUNC, 0x0040, 8,
                         &tmp[8] );
    ...
    if ( 1 == rwparm.isLevel_changed ) {
        //level
        QString qlevel = QString::number (
                             rwparm.lev.toFloat(), 'f', 1 );
        tmp[0] = ( u8 ) ( 0x00 ); //devno
        ...
        ret = writeREG ( REG64_BY_UDP_FUNC, 0x0048, 2,
                         &tmp[16] );
    ...
    if ( ( 1 == rwparm.isQam_sym_changed )
         || ( 1 == rwparm.isFreq_changed )
         || ( 1 == rwparm.isLevel_changed ) ) {
        u8 a = 0;
        ret = writeRdCmdFinish ( 0, &a, 1 );
    ...
    rwparm.isQam_sym_changed = 0;
    rwparm.isFreq_changed = 0;
    rwparm.isLevel_changed = 0;
    return ret;
}
int TBShardware::writeNetMac ( void )
{
    ...
    char charry[32] = { '\0' };
    strcpy ( charry,
             rwparm.netmac.toLatin1().data() );
    sscanf ( charry, "%02x:%02x:%02x:%02x:%02x:%02x"
             , &tmp[0], &tmp[1], &tmp[2], &tmp[3], &tmp[4],
             &tmp[5] );
    int ret = controlExternalMemory ( WRITE, 0x0008,
                                      tmp, 6 );
...
int TBShardware::udpMulticastClinet ( void )
{
    ...
    if ( ( Mfd = socket ( AF_INET, SOCK_DGRAM,
                          0 ) ) < 0 ) {
    ...
    addr.sin_addr.s_addr =
        inet_addr ( "239.255.0.9" );
    addr.sin_port = htons ( 21571 ); //0x5443
    len = sizeof ( addr );
    setsockopt ( Mfd, SOL_SOCKET, SO_RCVTIMEO,
                 ( char * ) &timeout, sizeof ( timeout ) );
    setsockopt ( Mfd, SOL_SOCKET, SO_SNDTIMEO,
                 ( char * ) &timeout, sizeof ( timeout ) );
    sendbuf[0] = 0xc0;//read
    sendbuf[1] = 0xff;
    sendbuf[2] = 0xff;
    ...
    if ( sendto ( Mfd, sendbuf, strlen ( sendbuf ), 0,
    ...
    while ( i < 16 ) {
        int n = recvfrom ( Mfd, recvbuf, 64, 0,
                           ( struct sockaddr * ) &addr, &len );

        if ( n < 0 ) {
            qDebug() <<
                     QString ( "%1->%2->%3:scan ip dev over" )
                     .arg ( __FILE__ ).arg ( __LINE__ ).arg (
                         __FUNCTION__ );
            ...
            /**
                ...
                continue;
            **/
            qDebug ( "%02x %02x %d.%d.%d %d",
                     ( u8 ) recvbuf[4], ( u8 ) recvbuf[5]
            ...
            for ( k = 0; k < gatewaynum; k++ ) {
                if ( true == hostip[k].contains (
                         QString ( "%1.%2.%3" )
                         .arg ( ( u8 ) recvbuf[6] ).arg ( ( u8 )
                                 recvbuf[7] )
                         .arg ( ( u8 ) recvbuf[8] ),
                         Qt::CaseSensitive ) ) {
                    break;
            ...
            tmp[0] = ( u8 ) recvbuf[11];
            tmp[1] = ( u8 ) recvbuf[10];
            tbsmsg->devip = QString ( "%1.%2.%3.%4" ).arg ( (
                                u8 ) recvbuf[6] )
                            .arg ( ( u8 ) recvbuf[7] )
                            .arg ( ( u8 ) recvbuf[8] )
                            .arg ( ( u8 ) recvbuf[9] );
            ...
            setudpfd ( mudpfd );
            ret = readREG ( REG64_BY_UDP_FUNC, 0x4014, 1,
                            &tbsmsg->switchStatus );
...
int TBShardware::getHostIpAddress ( void )
{
    ...
    for ( int i = 0; i < nListSize; ++i ) {
        if ( ipAddressesList.at ( i ) !=
             QHostAddress::LocalHost &&
             ipAddressesList.at ( i ).toIPv4Address() &&
             ( ipAddressesList.at ( i ) !=
               QHostAddress::Null ) ) {
            hostip[gatewaynum] = ipAddressesList.at (
                                     i ).toString();
            qDebug() << i << "local PC ip:" <<
                     hostip[gatewaynum];
            gatewaynum++;
        }
    }
...
int TBShardware::checkStatus_RX ( int times )
{
    ...
    for ( i = 0; i < times; i++ ) {
        ret = readREG ( REG64_BY_UDP_FUNC, 0x0040, 2,
                        checkstatus );
...
int TBShardware::checkStatus_TX ( int times )
{
    ...
    for ( i = 0; i < times; i++ ) {
        ret = readREG ( REG64_BY_UDP_FUNC, 0x0038, 2,
                        checkstatus );
...
int TBShardware::reset()
{
    ...
    ret = writeREG ( REG64_BY_UDP_FUNC, 0x0040, 1,
                     &tmp[8] );
...
void TBShardware::sethdudpfdMode ( SOCKET
                                   f_hdudpfd )
{
    setudpfd ( f_hdudpfd );
}
SOCKET TBShardware::openNetSocket ( QString ip,
                                    int port )
{
    return udpOpen ( ip, port );
}
SOCKET TBShardware::closeNetSocket (
    SOCKET socketfd )
{
...
int TBShardware::openNetSocket ( QString ip,
                                 int port )
{
    return udpOpen ( ip, port );
}
...
void TBShardware::setHardWareParm (
    RD_WT_PARM  rw )
{
    rwparm = rw;
}